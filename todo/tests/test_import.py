...
class ImportTodoCSVTestCase(TestCase):
    ...
    def test_successful_import_with_minimal_data(self):
        csv_file = self.generate_csv_file([
            ['Minimal List', 'Minimal Item', 'Minimal Text',
                'true', '2024-01-01', '2024-02-01']
        ])
        response = self.client.post(self.url, {'csv_file': csv_file})
        self.assertTrue(List.objects.filter(
            title_text="Minimal List").exists())
        self.assertTrue(ListItem.objects.filter(
            item_name="Minimal Item").exists())

    def test_successful_import_with_minimal_data(self):
        csv_file = self.generate_csv_file([
            ['Test List', 'Sample Item', 'Sample Text',
                'true', '2024-10-01', '2024-12-01']
        ])
        response = self.client.post(self.url, {'csv_file': csv_file})
        ...
        self.assertTrue(ListItem.objects.filter(
            item_name="Sample Item").exists())

    def test_blank_is_done_field(self):
        csv_file = self.generate_csv_file([
            ['List Blank Is Done', 'Item Blank Is Done',
                'Some Text', '', '2024-01-01', '2024-03-01']
        ])
        response = self.client.post(self.url, {'csv_file': csv_file})
    ...
    def test_case_insensitive_boolean_in_is_done_field(self):
        csv_file = self.generate_csv_file([
            ['Case Insensitive Boolean List', 'Boolean Item',
                'Boolean Text', 'TRUE', '2024-01-01', '2024-04-01']
        ])
        response = self.client.post(self.url, {'csv_file': csv_file})
    ...
    def test_numeric_title_text_field(self):
        csv_file = self.generate_csv_file([
            ['12345', 'Item Numeric Title', 'Text for Numeric Title',
                'false', '2024-01-01', '2024-04-01']
        ])
        response = self.client.post(self.url, {'csv_file': csv_file})
    ...
    def test_special_characters_in_item_name(self):
        csv_file = self.generate_csv_file([
            ['Special Char List', '@Special#Item!',
                'Some text here', 'true', '2024-01-01', '2024-05-01']
        ])
        response = self.client.post(self.url, {'csv_file': csv_file})
        self.assertTrue(ListItem.objects.filter(
            item_name="@Special#Item!").exists())

    def test_multiple_rows_in_csv(self):
        csv_file = self.generate_csv_file([
            ['Multi List', 'Item One', 'Text One',
                'true', '2024-01-01', '2024-04-01'],
            ['Multi List', 'Item Two', 'Text Two',
                'false', '2024-01-02', '2024-05-01']
        ])
        response = self.client.post(self.url, {'csv_file': csv_file})
        self.assertEqual(List.objects.filter(
            title_text="Multi List").count(), 1)
        self.assertEqual(ListItem.objects.filter(
            list__title_text="Multi List").count(), 2)

    def test_successful_import_with_valid_data(self):
        csv_file = self.generate_csv_file([
            ['Test List', 'Item 1', 'Text for item 1',
                'true', '2024-10-01', '2024-12-01']
        ])
        response = self.client.post(self.url, {'csv_file': csv_file})
    ...
    def test_import_with_all_false_is_done(self):
        csv_file = self.generate_csv_file([
            ['Test List', 'Item 1', 'Text for item 1',
                'false', '2024-10-01', '2024-12-01'],
            ['Test List', 'Item 2', 'Text for item 2',
                'false', '2024-10-01', '2024-12-01']
        ])
        response = self.client.post(self.url, {'csv_file': csv_file})
    ...
    def test_import_with_non_boolean_is_done(self):
        csv_file = self.generate_csv_file([
            ['Test List', 'Item 1', 'Text for item 1',
                'maybe', '2024-10-01', '2024-12-01']
        ])
        response = self.client.post(self.url, {'csv_file': csv_file})
        self.assertTrue(ListItem.objects.filter(
            item_name="Item 1", is_done=False).exists())

    def test_import_with_one_valid_row(self):
        csv_file = self.generate_csv_file([
            ['Test List', 'Single Item', 'Single Text',
                'true', '2024-10-01', '2024-12-01']
        ])
        response = self.client.post(self.url, {'csv_file': csv_file})
        self.assertTrue(ListItem.objects.filter(
            item_name="Single Item").exists())
